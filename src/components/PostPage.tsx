...
import { categoryLabels, localizePath, type Locale } from "@/lib/i18n";

export function PostPage({ slug, locale = "original" }: { slug: string; locale?: Locale }) {
  const post = getPostBySlug(slug, locale);

  if (!post) notFound();

  const categoryLabel = categoryLabels[locale][post.meta.category];
      ...
      <div className="mx-auto max-w-4xl px-6 md:px-10 lg:px-12">
        ...
        <div className="article-prose prose max-w-none rounded-3xl border border-border bg-[var(--article-surface)] px-4 py-6 shadow-[var(--card-shadow)] sm:px-6 sm:py-8 md:px-10 md:py-10">
          <ReactMarkdown
            ...
            components={{
              pre: ({ children }) => (
                <pre tabIndex={0} aria-label={locale === "en" ? "Code example" : "代码示例"}>{children}</pre>
              ),
              table: ({ children }) => (
                <div className="overflow-x-auto" tabIndex={0} role="region" aria-label={locale === "en" ? "Article table" : "文章表格"}>
